...
extern "C" {
#endif
int __EXPORT tx_begin ();
int __EXPORT tx_close ();
int __EXPORT tx_commit ();
int __EXPORT tx_info (TXINFO *);
int __EXPORT tx_open ();
int __EXPORT tx_rollback ();
int __EXPORT tx_set_commit_return (COMMIT_RETURN);
int __EXPORT tx_set_transaction_control (TRANSACTION_CONTROL);
int __EXPORT tx_set_transaction_timeout (TRANSACTION_TIMEOUT);
#if defined (__cplusplus)
}
#endif
