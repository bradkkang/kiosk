...
typedef struct
{
	...
	char menu_nm			[100+1];	///< 메뉴명
	char menu_step_sqno		[5+1];	///< 메뉴단계순번
	char menu_mrk_seq		[5+1];	///< 메뉴표시순서
	char cty_bus_user_dvs_cd[1+1];	///< 시외버스사용자구분코드 ('C':센터관리자, 'T':터미널관리자, 'B':운수사관리자, 'S':매표원)
	char hgrn_menu_id		[17+1];	///< 상위메뉴ID
	char menu_url_val		[100+1];	///< 메뉴URL값
} rtk_readmenucd_list_t, *prtk_readmenucd_list_t;

typedef struct
{
	char rsp_cd				[6+1];	///< 응답코드
	char menu_cd_num		[5+1];	///< 메뉴코드 수
	rtk_readmenucd_list_t* pList;	///< 메뉴코드 정보
} rtk_readmenucd_t, *prtk_readmenucd_t;
...
typedef struct
{
	...
	char trml_brn			[2+1];	///< 터미널사업자등록번호
	char cty_bus_area_cd	[2+1];	///< 시외버스지역코드
	char trml_drtn_cd		[2+1];	///< 터미널방면코드
	char snd_ptr_use_yn		[2+1];	///< 다른 프린터 사용 여부
...
} tk_rotfeedtl_list_t, *ptk_rotfeedtl_list_t;

typedef struct
{
	char rsp_cd				[6+1];	///< 응답코드
	char rot_fee_dtl_num	[5+1];	///< 노선요금상세 수
	tk_rotfeedtl_list_t	list[100+1];		
} tk_rotfeedtl_t, *ptk_rotfeedtl_t;
...
typedef struct
{
	...
	char alcn_sqno			[5+1];	///< 배차순번(number)
...
// 	int  n_pub_num			;		///< 발행매수 (number)				
// 	int  n_pub_sno			;		///< 발행일련번호 (number)			
// 	int  n_tisu_amt			;		///< 발권금액 (number)
// 	int  n_sats_no			;		///< 좌석번호 (number)			
// 	int  n_dc_bef_amt		;		///< 할인이전금액 (number)
...
typedef struct
{
	...
	int  n_card_aprv_amt		;		///< 카드승인금액	(number)
	int  n_frc_cmrt				;		///< 가맹점수수료율 (number)
	int  n_dist					;		///< 거리 (number)
	int  n_take_drtm			;		///< 소요시간	 (number)
	int  n_pub_num				;		///< 발행매수	 (number)	
	int  n_pub_sno				;		///< 발행일련번호	(number)	
...
typedef struct
{
	char req_pgm_dvs		 [3];	///< 요청 프로그램 구분	
	char req_trml_cd		 [7];	///< 요청 터미널 코드		
	char req_trml_wnd_no	 [2];	///< 요청 터미널 창구 번호	
	char req_trml_user_ip    [40];	///< 요청 터미널 사용자 IP	
	char req_trml_user_id    [17];	///< 요청 터미널 사용자 ID	
...
typedef struct
{
	char rsp_cd				[6+1];	///< 응답코드
	char damo_enc_dta_key	[32+1];	///< 암호문키
	char damo_enc_dta_key_len[2+1];	///< 암호문키길이
